...
class MicrosoftGraphService {
    ...
    async sendSubmissionEmail(payload) {
        const recipient = (0, formRegistry_js_1.resolveFormRecipient)(payload.formConfig);
        const subjectData = {
        ...
        if (this.isMockMode) {
            logger_js_1.Logger.info(`[MOCK GRAPH MODE] Simulating Microsoft Graph API email delivery`, {
        ...
        try {
            ...
            const draftMessage = await createRes.json();
            ...
            logger_js_1.Logger.info(`Microsoft Graph API email successfully sent`, { reference: payload.reference, messageId, recipient });
            return {
                success: true,
                messageId,
                mode: 'azure_graph',
            ...
            logger_js_1.Logger.error(`Microsoft Graph API submission failed`, { reference: payload.reference, error: err.message });
            return {
                success: false,
                error: err.message,
                mode: 'azure_graph',
    ...
    async uploadLargeAttachmentSession(accessToken, messageId, file) {
        ...
        const fileFd = fs_1.default.openSync(file.tempFilePath, 'r');
        ...
        try {
            while (start < totalSize) {
                ...
                fs_1.default.readSync(fileFd, chunkBuffer, 0, currentChunkLength, start);
